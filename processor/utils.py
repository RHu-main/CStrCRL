...
class Jointmask:
    ...
    def __call__(self, data, frame):
        N,C,T,V,M = data.shape
        data = data.permute((0,2,4,3,1))##N,T,M,V,C
        mask = torch.ones((T,V)) #50 25
        mask_joint_num = int(V*self.mask_ratio)
        for i in range(frame):
            rand = random.sample(range(0,V), mask_joint_num) # 在25个点里任意sample 2个点
            mask[i][rand] = 0.0 #筛选出来的点设为 0
        mask = mask.reshape(1, T, 1, V, 1).repeat(N, 1, M, 1, C) # N T M V C
        return mask
