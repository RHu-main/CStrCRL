...
class PT_Processor(Processor):
    ...
    def adjust_lr(self):
        if self.arg.optimizer == 'SGD' and self.arg.step:
            epoch = self.ep
            if epoch < self.arg.warmup_epochs:
                lr = self.arg.base_lr * epoch / self.arg.warmup_epochs 
            else:
                lr = self.arg.base_lr * (
            ...
            if epoch < self.arg.warmup_epochs:
                ...
                lr = self.arg.base_lr * 0.5 * (1. + math.cos(math.pi * (epoch - self.arg.warmup_epochs) / (self.arg.num_epoch - self.arg.warmup_epochs)))
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = lr
            self.lr = lr
        else:
