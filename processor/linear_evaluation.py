...
class LE_Processor(Processor):
    ...
    def train(self, epoch):
        ...
        for data, label in loader:
            # label = label.squeeze(1) # for pkummd dataset
            self.global_step += 1
            # get data
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--knn_interval', type=str, default=-1, nargs='+', help='knn_interval')
        ...
        parser.add_argument('--tsne_interval', type=str, default=-1, nargs='+', help='tsne_interval')
        return parser


class LE_Processor_ST(Processor):
    """
        Processor for Linear Evaluation.
    """
    ...
    def train(self, epoch):
        ...
        for [data, data_tem], label in loader:
            # label = label.squeeze(1) # pkummd dataset
            self.global_step += 1
            # get data
            data = data.float().to(self.dev, non_blocking=True)
            data_tem = data_tem.float().to(self.dev, non_blocking=True)
            label = label.long().to(self.dev, non_blocking=True)
            ...
            # forward
            output = self.model(None, data, data_tem)
            # print(output)
            # continue
            loss = self.loss(output, label)
