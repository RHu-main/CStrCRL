...
class Processor(IO):
    ...
    def load_data(self):
        ...
        if self.arg.test_feeder_args:
            ...
            self.data_loader['test'] = torch.utils.data.DataLoader(
                ...
                drop_last=False,
                worker_init_fn=init_seed)
        if self.arg.knn_train_feeder_args:
            knn_train_feeder = import_class(self.arg.knn_train_feeder)
            self.data_loader['knn_train'] = torch.utils.data.DataLoader(
                dataset=knn_train_feeder(**self.arg.knn_train_feeder_args),
                batch_size=self.arg.batch_size,
                shuffle=True,
        ...
        if self.arg.knn_test_feeder_args:
            knn_test_feeder = import_class(self.arg.knn_test_feeder)
            self.data_loader['knn_test'] = torch.utils.data.DataLoader(
                dataset=knn_test_feeder(**self.arg.knn_test_feeder_args),
                batch_size=self.arg.test_batch_size,
                shuffle=False,
                pin_memory=False,
                num_workers=self.arg.num_worker * torchlight.ngpu(
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if self.arg.eval_interval == -1:
                    ...
                    if self.current_result >= self.best_result:
                        ...
                        #     result_dict = dict(
                        #         zip(self.data_loader['test'].dataset.label,
                        #             self.result))
                ...
                if self.arg.knn_interval == -1:
                    ...
                    for k in self.arg.knn_k:
                        ...
                        if epoch + 1 in self.arg.KNN_show:
                            ...
                            self.KNN_epoch_results[epoch + 1][k] = [self.knn_results[k][epoch + 1],
                                                                    max(self.knn_results[k].values())]
                ...
                if self.arg.tsne_interval == -1:
                    pass
                elif ((epoch + 1) % self.arg.tsne_interval == 0) or (epoch + 1 == self.arg.num_epoch):
                    self.io.print_log('tsne feature map epoch {}'.format(epoch + 1))
                    self.tsne_monitor(epoch + 1)
                    print('-------tsne feature map already saving-------')
        # test phase
            ...
            if self.arg.tsne_interval == -1:
                ...
                self.tsne_monitor(epoch+1)
                print('-------tsne feature map already saving-------')
            if self.arg.tsne_pn_aug == -1:
                pass
            elif epoch + 1 == self.arg.num_epoch:
                self.tsne_pn_monitor(epoch+1)
    ...
    def get_parser(add_help=False):
        ...
        # feeder
        parser.add_argument('--knn_train_feeder', default='feeder.feeder', help='train data loader will be used')
        ...
        parser.add_argument('--knn_test_feeder', default='feeder.feeder', help='test data loader will be used')
        parser.add_argument('--num_worker', type=int, default=4, help='the number of worker per gpu for data loader')
        parser.add_argument('--knn_train_feeder_args', action=DictAction, default=dict(), help='the arguments of data loader for training')
        parser.add_argument('--train_feeder_args', action=DictAction, default=dict(), help='the arguments of data loader for training')
        parser.add_argument('--knn_test_feeder_args', action=DictAction, default=dict(), help='the arguments of data loader for test')
